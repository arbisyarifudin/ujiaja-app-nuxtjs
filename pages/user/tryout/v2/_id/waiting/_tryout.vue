  ...
  methods: {
    ...
    toTryoutTestPage(productID, tryoutID) {
      ...
      window.location.replace(
        `/user/tryout/v2/${encryptedProductIdSafe}/test/${encryptedTryoutIdSafe}?kode=${this.$route.query.kode}${this.$route.query.id_mapel ? '&id_mapel=' + this.$route.query.id_mapel : ''}`
      );
    },
    onCloseWindow(event) {
