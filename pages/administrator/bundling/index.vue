    ...
    <div class="row d-flex no-gutters">
      ...
      <div class="col-md-12 text-right mt-4 crud-tools">
        <div class="row no-gutters justify-content-between">
          ...
          <div class="col-md-3 d-flex align-items-center crud-tools">
            <b-input-group>
              <template #prepend>
                <b-input-group-text><i class="fas fa-search"></i></b-input-group-text>
              </template>
              <b-form-input v-model="filter.keyword" placeholder="Cari bundle" debounce="1000"></b-form-input>
            </b-input-group>
            <nuxt-link class="btn btn-primary tambah crud-btn__add px-4 ml-2" to="/administrator/bundling/add">
              Tambah
            </nuxt-link>
          </div>
        </div>
            ...
            <div class="card card-karir m-2 router-push"
              ...
              <div class="card-body text-left p-0">
                <span style="top: 14px; left: 14px; z-index: 99; position: absolute;cursor: pointer;"
                  @click.stop.prevent="archiveBundlingTryOut(`${item.id}`)">
                  <img :src="`${item.is_archived_admin}` == '1' ? '/icon/archive-tick.svg' : '/icon/unarchive-tick.svg'"
                    class="img-fluid image" />
                </span>
                <div class="" style="display: flex; justify-content: flex-end;">
                  <p class="label-event mb-2 px-4 py-1" :class="[item.show ? '' : 'draft']">
                ...
                <div class="card-content px-4">
                  ...
                  <div class="package-getlist mb-3"
                    v-html="item.desc && item.desc.toString().substring(0, 50) + ' ...'"></div>
                </div>
                <div class="card-bawah pb-3 px-4 m-0 bordered"
                  style="border-bottom: 12px solid #D7D2F7; border-radius:0px 0px 12px 12px;">
                  <nuxt-link :to="`/administrator/bundling/${item.id}/detail`" class="karir-link">Detail <i
      ...
      <div v-if="totalRows == 0 && !loading && !filter.keyword" class="crud-body kosong">
        ...
        <nuxt-link class="btn btn-primary tambah-kelas px-4 py-3" to="/administrator/bundling/add">
          Tambah Produk
        </nuxt-link>
      </div>
    </div>
...
<script>
  export default {
    ...
    methods: {
      archiveBundlingTryOut(id) {
        this.loading = true
        this.$axios
          .$put(`api/bundling/archived/${id}`)
          .then(res => {
            this.loading = false
            this.getData('bundling')
            return true;
